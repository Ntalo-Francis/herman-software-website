    ...
    <header className="sticky top-0 z-50 border-b border-gray-light bg-white/95 backdrop-blur-sm">
      <div className="container-site">
        <div className="flex h-16 items-center justify-between">
          {/* Logo */}
          <a href="/" className="flex items-center gap-3" aria-label="HERMAN Software Solutions">
                  ...
                  >
                    <button
                      className={cn(
                        ...
                        servicesOpen ? "text-teal" : "text-charcoal hover:text-teal"
                      )}
                    >
                      {link.label}
                      ...
                      <div className="absolute left-0 top-full mt-1 w-56 rounded-card border border-gray-light bg-white py-2 shadow-card">
                        {link.children.map((child) => (
                          <a
                            ...
                            href={child.href}
                            className={cn(
                              "block px-4 py-2 text-body-sm transition-colors hover:bg-gray-light hover:text-teal",
                              isActive(child.href) && "font-medium text-teal"
                            )}
                          >
                            {child.label}
                          </a>
              ...
              return (
                <a
                  ...
                  className={cn(
                    ...
                    isActive(link.href) ? "text-teal" : "text-charcoal hover:text-teal"
                  )}
                >
                  {link.label}
                </a>
          ...
          <div className="hidden lg:flex lg:items-center lg:gap-3">
            <a
              href="/client-portal"
              className="rounded-md border border-navy px-4 py-2 text-body-sm font-medium text-navy transition-colors hover:bg-navy hover:text-white"
            >
              Client Portal
            </a>
            <a
              href="/get-quote"
              className="rounded-md bg-teal px-4 py-2 text-body-sm font-medium text-white transition-colors hover:bg-teal-dark"
            >
              Get a Quote
            </a>
          </div>
      ...
      {mobileOpen && (
        <div className="fixed inset-0 top-16 z-40 bg-white lg:hidden">
          <nav className="container-site flex flex-col gap-2 py-6" aria-label="Mobile navigation">
            {NAV_LINKS.map((link) => {
              if ("children" in link && link.children) {
                return (
                  <div key={link.label} className="border-b border-gray-light pb-2">
                    <div className="px-4 py-2 text-body-sm font-semibold text-charcoal">
                      {link.label}
                    </div>
                    {link.children.map((child) => (
                      <a
                        ...
                        className={cn(
                          "block px-6 py-2 text-body-sm transition-colors hover:text-teal",
                          isActive(child.href) && "font-medium text-teal"
                        )}
                      >
                        {child.label}
              ...
              return (
                <a
                  ...
                  onClick={() => setMobileOpen(false)}
                  className={cn(
                    "rounded-md px-4 py-3 text-body font-medium transition-colors",
            ...
            <div className="mt-4 flex flex-col gap-3 border-t border-gray-light pt-6">
              <a
                href="/client-portal"
                onClick={() => setMobileOpen(false)}
